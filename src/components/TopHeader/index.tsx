...
import { EmailIcon, EmailShareButton, FacebookIcon, FacebookMessengerShareButton, MailruIcon, MailruShareButton, RedditIcon, RedditShareButton, TelegramIcon, TelegramShareButton, TwitterIcon, TwitterShareButton, ViberIcon, ViberShareButton, VKIcon, VKShareButton, WhatsappIcon, WhatsappShareButton } from "react-share";
...
const TopHeader = () => {
    ...
    const GetBiggestTxOfDay = async () => {
        ...
        const resData = [...btc.data.result, ...eth.data.result, ...usdc.data.result, ...usdt.data.result,...bnb.data.result]
        if (resData) {
            const today = new Date()
            const day = today.getDate()
            const month = today.getMonth()
            const year = today.getFullYear()
            const allNeedDate = `${day < 10 ? '0' : ''}${day}.${month + 1 < 10 ? '0' : ''}${month + 1}.${year}`
            let check = resData[0]
        ...
        <div className="max-w-lg bg-white p-8 rounded-r-2xl flex flex-col justify-between ">
            <div className="relative">
                <img
                    ...
                    className=""
                />
                <div className="flex items-center gap-3 ml-auto nav_share text-white" onMouseMove={() => setShare(true)} onMouseLeave={CheckShare}>
                    <svg
                        width="35px"
                        height="35px"
                        viewBox="0 -0.5 25 25"
                ...
                <div className={`nav_transaction_popup`} id={share ? "active_share_popup" : "none_share_popup"} onMouseLeave={CloseShare} onMouseMove={ShareTransaction}>
                    <FacebookMessengerShareButton url={url} appId="1">
                        <FacebookIcon className={`share_bttn_icon ${share ? "acive_s_bttn" : "none_s_bttn"}`} />
                    </FacebookMessengerShareButton>
                    <TelegramShareButton url={url}>
                        <TelegramIcon className={`share_bttn_icon ${share ? "acive_s_bttn" : "none_s_bttn"}`} />
                    </TelegramShareButton>
                    <WhatsappShareButton url={url}>
                        <WhatsappIcon className={`share_bttn_icon ${share ? "acive_s_bttn" : "none_s_bttn"}`} />
                    </WhatsappShareButton>
                    <EmailShareButton url={url}>
                        <EmailIcon className={`share_bttn_icon ${share ? "acive_s_bttn" : "none_s_bttn"}`} />
                    </EmailShareButton>
                    <TwitterShareButton url={url}>
                        <TwitterIcon className={`share_bttn_icon ${share ? "acive_s_bttn" : "none_s_bttn"}`} />
                    </TwitterShareButton>
                    <RedditShareButton url={url}>
                        <RedditIcon className={`share_bttn_icon ${share ? "acive_s_bttn" : "none_s_bttn"}`} />
                    </RedditShareButton>
                    <VKShareButton url={url}>
                        <VKIcon className={`share_bttn_icon ${share ? "acive_s_bttn" : "none_s_bttn"}`} />
                    </VKShareButton>
                    <ViberShareButton url={url}>
                        <ViberIcon className={`share_bttn_icon ${share ? "acive_s_bttn" : "none_s_bttn"}`} />
                    </ViberShareButton>
                    <MailruShareButton url={url}>
                        <MailruIcon className={`share_bttn_icon ${share ? "acive_s_bttn" : "none_s_bttn"}`} />
                    </MailruShareButton>
                </div>
            </div>
                    ...
                    >
                        <svg xmlns="http://www.w3.org/2000/svg" width="54" height="54" viewBox="0 0 54 54" fill="none">
                            <g clipPath="url(#clip0_612_14)">
                                <path d="M39.2141 0H14.7859C6.61988 0 0 6.61988 0 14.7859V39.2141C0 47.3801 6.61988 54 14.7859 54H39.2141C47.3801 54 54 47.3801 54 39.2141V14.7859C54 6.61988 47.3801 0 39.2141 0Z" fill="url(#paint0_radial_612_14)" fillOpacity="0.7" />
